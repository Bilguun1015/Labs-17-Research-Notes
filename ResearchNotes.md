...
| attendance_crosstab_summary | Groups attendance by course_id and student_id, displays course_id, student_id, first_name, count of attendance A as A, AE as AE, X is X and L as L.  | SELECT mt.course_id, s.student_id, s.first_name, COUNT(case when att.attendance = 'A' then 1 else null end) as "A", COUNT(case when attendance = 'AE' then 1 else null end) as "AE", COUNT(case when attendance = 'X' then 1 else null end) as "X", COUNT(case when attendance = 'L' then 1 else null end) as "L" FROM "Student" as s INNER JOIN ("Meeting" as mt INNER JOIN "Attendance" as att ON mt.meeting_id = att.meeting_id) ON s.student_id = att.student_id GROUP BY mt.course_id, s.student_id, s.first_name; | Meeting = mt, Student = s, Attendance = att | none |
| attending_by_term | CE.course_id, CE.student_id, S.first_name, S.additional_names, S.mobile_telephone, CE.result, GC.status, GC.term, GC.days, GE.notes, X, L | SELECT mt.course_id, s.student_id, s.first_name, s.additional_names, s.mobile_telephone, ce.result, gc.status, gc.term, gc.days, ce.notes, COUNT(case when att.attendance = 'X' then 1 else null end) as "X", COUNT(case when attendance = 'L' then 1 else null end) as "L" FROM "Student" as s INNER JOIN ("Meeting" as mt INNER JOIN "Attendance" as att ON mt.meeting_id = att.meeting_id) ON s.student_id = att.student_id LEFT JOIN ("CourseEnrollment" as ce LEFT JOIN "GeneralCourse" as gc ON ce.course_id = gc.course_id) ON s.student_id = ce.student_id WHERE (((ce.result) Is Null) AND ((gc.status) In ('active','completed')) AND ((gc.term)=17)) OR (((ce.result) Not In (1,0,-1,-2)) AND ((gc.status) In ('active','completed')) AND ((gc.term)=17)) GROUP BY mt.course_id, s.student_id, s.first_name, ce.result, gc.status, gc.term, gc.days, ce.notes; | CE = CourseEnrollement, S = Student, GC = GeneralCourse | attendance_crosstab_summary |
